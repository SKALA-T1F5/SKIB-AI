...
def get_collection_info(collection_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if data["metadatas"]:
            ...
            metadata_stats = {"types": type_counts, "sources": source_counts}
...
def clear_collection(collection_name: str) -> bool:
    ...
    try:
        ...
        if data["ids"]:
            ...
            logger.info(
                f"🧹 컬렉션 '{collection_name}' 내용 삭제: {len(data['ids'])}개"
            )
...
def get_collection_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"📈 ChromaDB 통계: {len(collections)}개 컬렉션, {total_documents}개 문서"
        )
        return stats
