...
class ChromaDBPipeline:
    ...
    def process_and_upload_document(
        ...
        try:
            ...
            result = {
                ...
                "upload_success_rate": uploaded_count / len(document_blocks) if document_blocks else 0
            }
            
            logger.info(f"✅ 문서 업로드 완료: {uploaded_count}/{len(document_blocks)}개")
            return result
    ...
    def bulk_process_documents(
        ...
        for i, doc in enumerate(documents):
            try:
                ...
                collection_name = doc.get("collection_name", base_collection_name or f"collection_{i}")
        ...
        logger.info(f"📊 일괄 처리 완료: {success_count}/{len(documents)}개 문서, {total_uploaded}/{total_blocks}개 블록")
    ...
    def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                "client_status": "connected" if self.client.test_connection() else "disconnected",
...
def quick_upload(
    ...
    return pipeline.process_and_upload_document(document_blocks, collection_name, source_file)
...
def quick_search(
    ...
    """빠른 검색"""
    pipeline = ChromaDBPipeline()
    result = pipeline.search_and_analyze(query, collection_name, n_results, include_analysis=False)
