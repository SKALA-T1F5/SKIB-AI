...
class ChromaDBDeleter:
    ...
    def delete_multiple_collections(
        self, collection_names: List[str], force: bool = False
    ) -> Dict[str, bool]:
        """
        여러 컬렉션 일괄 삭제
    ...
    def delete_collections_by_pattern(
        self, pattern: str, force: bool = False
    ) -> Dict[str, bool]:
        """
        패턴에 맞는 컬렉션들 삭제
        ...
        logger.info(
            f"🔍 패턴 '{pattern}'에 맞는 컬렉션 {len(matching_collections)}개: {matching_collections}"
        )
...
def delete_multiple_collections(
    collection_names: List[str], force: bool = False
) -> Dict[str, bool]:
    """여러 컬렉션 삭제 편의 함수"""
    deleter = ChromaDBDeleter()
