...
def example_pipeline_usage():
    ...
    result = pipeline.process_and_upload_document(
        document_blocks, collection_name, "pipeline_test.txt", recreate_collection=True
    )
    ...
    search_result = pipeline.search_and_analyze(
        "파이프라인", collection_name, n_results=2
    )
...
def example_advanced_search():
    ...
    metadata_results = searcher.search_by_metadata(
        collection_name, where={"chunk_type": "heading"}
    )
    ...
    for i, result in enumerate(hybrid_results, 1):
        print(
            f"  [{i}] {result['content'][:50]}... (유사도: {result['similarity']:.3f})"
        )
