...
from db.vectorDB.chromaDB import (
    get_client, list_collections, search_similar, 
    get_collection_info
)
...
from db.vectorDB.chromaDB.delete import (
    delete_collection, delete_multiple_collections, 
    delete_empty_collections, show_deletion_preview
)
...
def show_collection_contents():
    ...
    try:
        ...
        for i in range(len(data['documents'])):
            ...
            if metadata:
                # 주요 메타데이터만 표시
                important_keys = ['source', 'chunk_type', 'page', 'section', 'block_index']
                shown_meta = {k: v for k, v in metadata.items() if k in important_keys}
                if shown_meta:
                    print(f"🏷️  메타데이터:")
...
def delete_multiple_collections_interactive(collections):
    ...
    try:
        indices_input = input("\n삭제할 컬렉션 번호들을 쉼표로 구분해서 입력하세요 (예: 1,3,5): ").strip()
        ...
        # 최종 확인
        confirm = input(f"\n{len(selected_collections)}개 컬렉션을 삭제하려면 'DELETE'를 입력하세요: ").strip()
        if confirm != "DELETE":
            print("❌ 삭제가 취소되었습니다.")
            return
...
def delete_empty_collections_interactive():
    ...
    try:
        ...
        # 확인
        confirm = input(f"\n{len(empty_collections)}개 빈 컬렉션을 삭제하시겠습니까? (y/N): ").strip().lower()
        if confirm != 'y':
            print("❌ 삭제가 취소되었습니다.")
            return
...
def delete_test_collections_interactive():
    ...
    try:
        ...
        # 확인
        confirm = input(f"\n{len(test_collections)}개 테스트 컬렉션을 삭제하려면 'DELETE'를 입력하세요: ").strip()
        if confirm != "DELETE":
            print("❌ 삭제가 취소되었습니다.")
            return
