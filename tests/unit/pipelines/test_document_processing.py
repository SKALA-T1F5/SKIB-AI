...
@pytest.fixture(scope="module")
def document_data():
    return {"project_id": "1", "document_id": "123", "name": "test.pdf"}
...
def test_processing_status_immediate(api_base_url, document_data):
    response = requests.get(
        f"{api_base_url}/document/summary/{document_data['document_id']}"
    )
    assert response.status_code in (
        200,
        202,
        404,
    )  # Acceptable: ready, processing, or not found
...
def test_document_summary_success(api_base_url, document_data, test_file_path):
    ...
    with open(test_file_path, "rb") as f:
        ...
        upload_response = requests.post(
            f"{api_base_url}/document", files=files, params=document_data
        )
