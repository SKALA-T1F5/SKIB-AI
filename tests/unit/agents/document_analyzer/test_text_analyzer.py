...
class TestTextAnalyzer:
    ...
    async def test_extract_simple_text(self, analyzer, sample_blocks):
        ...
        with patch.object(analyzer, 'extract_structured_text', new_callable=AsyncMock) as mock_extract:
    ...
    async def test_extract_structured_text_with_docling_success(self, analyzer):
        ...
        with patch('src.agents.document_analyzer.tools.text_analyzer.DocumentConverter') as mock_converter:
            ...
            with patch('os.path.exists', return_value=True):
                ...
                assert result[0]["metadata"]["extraction_method"] == "docling_structured"
    ...
    async def test_extract_structured_text_docling_fallback(self, analyzer):
        ...
        with patch('src.agents.document_analyzer.tools.text_analyzer.DocumentConverter') as mock_converter:
            ...
            # pdfplumber Mock 설정
            mock_page = Mock()
            mock_page.extract_text.return_value = "pdfplumber로 추출된 텍스트입니다.\n\n이것은 두 번째 문단입니다."
            ...
            with patch('pdfplumber.open', return_value=mock_pdf):
                with patch('os.path.exists', return_value=True):
                    ...
                    assert result[0]["metadata"]["extraction_method"] == "pdfplumber_fallback"
