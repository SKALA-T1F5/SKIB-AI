...
class TestDocumentAnalyzerAgent:
    ...
    def test_analyze_document_success(self, mock_parse, sample_pdf_path, sample_blocks):
        ...
        mock_parse.assert_called_once_with(
            sample_pdf_path, "test_collection", generate_questions=False
        )

    @patch("src.agents.document_analyzer.agent.parse_pdf_unified")
    def test_analyze_document_with_questions(
        self, mock_parse, sample_pdf_path, sample_blocks
    ):
        """질문 생성과 함께 문서 분석 테스트"""
        # Mock 설정
        ...
        ) as mock_question_gen, patch.object(
            agent, "text_analyzer"
        ) as mock_text_analyzer:

            mock_question_gen.generate_questions_for_blocks.return_value = (
