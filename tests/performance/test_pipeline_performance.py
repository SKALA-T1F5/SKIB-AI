...
class TestPipelinePerformance:
    ...
    async def test_memory_usage_during_processing(
        ...
        mock_design_result = {
            "output": {
                "requirements": {},
                "test_summary": "요약",
                "test_config": {"num_questions": 5, "num_objective": 3, "num_subjective": 2}
            }
        }
        ...
        with patch('src.pipelines.test_generation.pipeline.DocumentAnalyzerAgent', return_value=mock_doc_agent), \
             ...
             patch.object(pipeline, '_upload_to_vectordb', return_value=100), \
        ...
        assert memory_increase < 100, f"메모리 사용량이 너무 많이 증가했습니다: {memory_increase:.2f}MB"
    ...
    async def test_processing_time_large_document(
        ...
        mock_design_result = {
            "output": {
                "requirements": {},
                "test_summary": "요약",
                "test_config": {"num_questions": 10, "num_objective": 6, "num_subjective": 4}
            }
        }
        mock_test_designer = AsyncMock()
        mock_test_designer.execute.return_value = mock_design_result
        ...
        with patch('src.pipelines.test_generation.pipeline.DocumentAnalyzerAgent', return_value=mock_doc_agent), \
             patch('src.pipelines.test_generation.pipeline.TestDesignerAgent', return_value=mock_test_designer), \
             patch('src.agents.question_generator.generate_questions.generate_question', return_value=sample_questions), \
             patch('openai.ChatCompletion.acreate', return_value=mock_openai_response), \
             patch.object(pipeline, '_upload_to_vectordb', return_value=len(large_blocks)), \
             patch.object(pipeline, '_save_results'):
        ...
        assert processing_time < 10, f"처리 시간이 너무 오래 걸렸습니다: {processing_time:.2f}초"
    ...
    async def test_concurrent_pipeline_execution(
        ...
        mock_doc_state.get_statistics.return_value = {"total_blocks": len(sample_blocks)}
        ...
        mock_design_result = {
            "output": {
                "requirements": {},
                "test_summary": "요약",
                "test_config": {"num_questions": 3, "num_objective": 2, "num_subjective": 1}
            }
        }
        mock_test_designer = AsyncMock()
        mock_test_designer.execute.return_value = mock_design_result
        ...
        async def run_single_pipeline(pipeline, collection_name):
            with patch('src.pipelines.test_generation.pipeline.DocumentAnalyzerAgent', return_value=mock_doc_agent), \
                 ...
                 patch('src.agents.question_generator.generate_questions.generate_question', return_value=sample_questions), \
                 patch('openai.ChatCompletion.acreate', return_value=mock_openai_response), \
                 patch.object(pipeline, '_upload_to_vectordb', return_value=3), \
                 patch.object(pipeline, '_save_results'):
        ...
        for i, result in enumerate(results):
            assert not isinstance(result, Exception), f"파이프라인 {i} 실행 실패: {result}"
            assert result["status"] == "completed"
        
        # 동시 실행 시간이 순차 실행보다 빨라야 함 (3배 이하)
        assert execution_time < 15, f"동시 실행 시간이 너무 오래 걸렸습니다: {execution_time:.2f}초"
    ...
    async def test_memory_cleanup_after_processing(
        ...
        with patch('src.pipelines.test_generation.pipeline.DocumentAnalyzerAgent', return_value=mock_doc_agent), \
             ...
             patch('src.agents.question_generator.generate_questions.generate_question', return_value=sample_questions), \
             patch('openai.ChatCompletion.acreate', return_value=mock_openai_response), \
             patch.object(pipeline, '_upload_to_vectordb', return_value=60), \
             patch.object(pipeline, '_save_results'):
        ...
        assert memory_difference < 50, f"메모리 누수 가능성: {memory_difference:.2f}MB 증가"
    ...
    async def test_error_recovery_performance(
        ...
        assert error_handling_time < 5, f"에러 처리 시간이 너무 오래 걸렸습니다: {error_handling_time:.2f}초"
    ...
    async def test_vectordb_upload_performance(
        ...
        with patch('src.pipelines.test_generation.pipeline.upload_chunk_to_collection') as mock_upload:
        ...
        # 업로드 성능 확인
        assert upload_time < 10, f"VectorDB 업로드 시간이 너무 오래 걸렸습니다: {upload_time:.2f}초"
        
        # 텍스트가 있는 블록만 업로드되었는지 확인
        expected_uploads = len([b for b in large_blocks if pipeline._extract_text_from_block(b)])
    ...
    async def test_question_generation_scalability(
        ...
        with patch('src.agents.question_generator.generate_questions.generate_question') as mock_generate, \
             patch.object(pipeline, '_prepare_vision_chunks') as mock_prepare:
            
            # 다수의 청크 시뮬레이션
            mock_prepare.return_value = [
                {"messages": [{"type": "text", "text": f"청크 {i}"}], "metadata": {"page": i}}
        ...
        assert generation_time < 15, f"질문 생성 시간이 너무 오래 걸렸습니다: {generation_time:.2f}초"
...
class TestMemoryProfiler:
    ...
    def test_memory_usage_baseline(self):
        ...
        assert baseline_memory < 500, f"기본 메모리 사용량이 너무 높습니다: {baseline_memory:.2f}MB"
    ...
    def test_pipeline_instantiation_memory(self):
        ...
        assert memory_increase < 50, f"파이프라인 인스턴스 생성으로 메모리가 너무 많이 증가했습니다: {memory_increase:.2f}MB"
