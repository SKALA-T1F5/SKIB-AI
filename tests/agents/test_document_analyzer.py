...
class TestDocumentAnalyzerAgent:
    ...
    @patch('src.agents.document_analyzer.tools.keyword_summary.extract_keywords_and_summary')
    def test_analyze_document_success(self, mock_extract_keywords, mock_parse_pdf, agent, sample_blocks):
        """문서 분석 성공 테스트"""
        # Mock 설정
    ...
    @patch('src.agents.document_analyzer.agent.parse_pdf_unified')
    def test_analyze_document_without_keywords(self, mock_parse_pdf, agent, sample_blocks):
        """키워드 추출 없이 문서 분석 테스트"""
        mock_parse_pdf.return_value = sample_blocks
    ...
    def test_parse_structure_only(self, mock_parse_pdf, agent, sample_blocks):
        ...
        mock_parse_pdf.assert_called_once_with("test.pdf", "test_collection", generate_questions=False)

    def test_analyze_text_only(self, agent):
        """텍스트 분석만 수행 테스트"""
        # analyze_text 메서드가 존재하지 않으므로 mock으로 테스트 (create=True 옵션 사용)
        with patch.object(agent.text_analyzer, 'analyze_text', create=True, return_value={"analysis": "result"}):
    ...
    def test_upload_to_chromadb_success(self, mock_upload, agent, sample_blocks):
        ...
        assert result == 4
        mock_upload.assert_called_once_with(sample_blocks, "test_collection", "test.pdf")
...
class TestDocumentAnalyzerIntegration:
    ...
    def test_full_workflow_mock(self):
        """전체 워크플로우 모의 테스트"""
        with patch('src.agents.document_analyzer.agent.parse_pdf_unified') as mock_parse, \
             patch('src.agents.document_analyzer.tools.keyword_summary.extract_keywords_and_summary') as mock_extract, \
             patch.object(DocumentAnalyzerAgent, '_save_results'), \
             patch.object(DocumentAnalyzerAgent, '_upload_to_chromadb', return_value=0):
