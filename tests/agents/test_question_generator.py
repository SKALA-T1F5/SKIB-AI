...
from src.agents.question_generator.agent import (
...
class TestQuestionGeneratorAgent:
    ...
    def test_generate_questions_from_blocks_success(
        self, mock_question_generator_class, agent, sample_blocks
    ):
        """블록으로부터 문제 생성 성공 테스트"""
        ...
        with patch.object(agent, "_save_question_results") as mock_save:
            mock_save.return_value = {
                "status": "completed",
                "questions": [
                    sample_blocks[0]["questions"][0],
                    sample_blocks[1]["questions"][0],
                ],
                "total_questions": 2,
    ...
    @patch("src.agents.question_generator.agent.QuestionGenerator")
    def test_generate_questions_from_blocks_failure(
        self, mock_question_generator_class, agent
    ):
        """블록으로부터 문제 생성 실패 테스트"""
        mock_generator = Mock()
        mock_question_generator_class.return_value = mock_generator
        mock_generator.generate_questions_for_blocks.side_effect = Exception(
    ...
    @patch("os.makedirs")
    @patch("json.dump")
    def test_save_question_results(
        self, mock_json_dump, mock_makedirs, mock_open_func, agent, sample_questions
    ):
        """문제 생성 결과 저장 테스트"""
        with patch.object(
            agent, "_save_test_summary", return_value="summary.json"
    ...
    @patch("os.makedirs")
    @patch("json.dump")
    def test_save_test_summary_success(
        self, mock_json_dump, mock_makedirs, mock_open_func, agent, sample_questions
    ):
        """테스트 요약 저장 성공 테스트"""
    ...
    @patch("os.makedirs")
    @patch("json.dump")
    def test_save_test_summary_failure(
        self, mock_json_dump, mock_makedirs, mock_open_func, agent
    ):
        """테스트 요약 저장 실패 테스트"""
    ...
    @patch("os.makedirs")
    @patch("json.dump")
    def test_save_test_config_success(
        self, mock_json_dump, mock_makedirs, mock_open_func, agent, sample_questions
    ):
        """테스트 설정 저장 성공 테스트"""
    ...
    @patch("os.makedirs")
    @patch("json.dump")
    def test_save_test_config_failure(
        self, mock_json_dump, mock_makedirs, mock_open_func, agent
    ):
        """테스트 설정 저장 실패 테스트"""
        ...
        result = agent._save_test_config(
            questions=[], source_file="test.pdf", timestamp="20240101_120000"
        )
    ...
    def test_analyze_content_complexity(self, agent):
        ...
        result = agent._analyze_content_complexity(
            keywords=["key"] * 15, main_topics=["topic"] * 8, questions=questions_hard
        )
        assert result == "고급"
        ...
        result = agent._analyze_content_complexity(
            keywords=["key"] * 7, main_topics=["topic"] * 4, questions=questions_normal
        )
        assert result == "중급"
        ...
        result = agent._analyze_content_complexity(
            keywords=["key"] * 3, main_topics=["topic"] * 2, questions=questions_easy
        )
        assert result == "초급"
...
class TestQuestionGeneratorIntegration:
    ...
    def test_error_handling(self):
        ...
        with patch.object(
            agent.question_generator, "generate_questions_for_blocks"
        ) as mock_generate:
    ...
    def test_empty_blocks_handling(self):
        ...
        with patch.object(
            agent.question_generator, "generate_questions_for_blocks", return_value=[]
        ), patch.object(agent, "_save_question_results") as mock_save:
