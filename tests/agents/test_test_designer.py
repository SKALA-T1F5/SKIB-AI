...
class TestTestDesignerAgent:
    ...
    async def test_act_success(self, agent, sample_input_data, sample_state):
        """테스트 설계 실행 성공 테스트"""
        # Mock methods
        with patch.object(agent, "_analyze_requirements") as mock_analyze, patch.object(
            agent, "_generate_test_summary"
        ) as mock_summary, patch.object(
            agent, "_create_test_config"
            ...
            mock_summary.return_value = {
                "test_title": "AI 기초 테스트",
                "test_summary": "AI 기초 평가",
            }
    ...
    async def test_generate_test_summary_success(self, agent):
        ...
        mock_response.choices[0].message.content = (
            '{"test_title": "AI 기초 테스트", "test_summary": "AI 기초 개념 평가"}'
        )
...
class TestTestDesignerIntegration:
    ...
    async def test_full_workflow_mock(self):
        ...
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = (
            '{"test_title": "테스트", "test_summary": "테스트 요약"}'
        )

        with patch("openai.AsyncOpenAI") as mock_openai, patch.object(
            agent, "update_progress"
        ):
