...
from src.pipelines.test_generation.pipeline import (
    TestGenerationPipeline,
    generate_test_from_document,
)
...
class TestTestGenerationPipeline:
    ...
    async def test_step1_document_analysis(
        self, mock_pdf_file, sample_blocks, mock_embedding_model
    ):
        """1단계 문서 분석 테스트"""
        pipeline = TestGenerationPipeline()
    ...
    async def test_step3_question_generation(
        self, mock_generate_question, sample_blocks, sample_questions
    ):
        """3단계 질문 생성 테스트"""
        pipeline = TestGenerationPipeline()
    ...
    def test_extract_text_from_block_paragraph(self):
        ...
        block = {"type": "paragraph", "content": "이것은 테스트 문단입니다."}
    ...
    async def test_upload_to_vectordb(
        self, mock_upload_chunk, sample_blocks, mock_embedding_model
    ):
        """VectorDB 업로드 테스트"""
        pipeline = TestGenerationPipeline()
        pipeline.embedding_model = mock_embedding_model
...
class TestPipelineErrorHandling:
    ...
    async def test_test_design_failure(
        self, mock_pdf_file, sample_user_prompt, sample_blocks
    ):
        """테스트 설계 실패 처리 테스트"""
        pipeline = TestGenerationPipeline()
...
class TestPipelinePerformance:
    ...
    async def test_processing_time_tracking(
        ...
        with patch(
            ...
            pipeline, "_upload_to_vectordb", return_value=1
        ), patch.object(
