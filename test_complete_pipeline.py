...
def test_document(pdf_path: str, document_name: str) -> Dict:
    ...
    try:
        ...
        # 2. 블록 분석
        text_blocks = [
            b for b in blocks if b.get("type") in ["paragraph", "section", "heading"]
        ]
        ...
        result = {
            ...
            "content_summary": {
                "summary": (
                    content_analysis.get("summary", "")[:100] + "..."
                ...
                "technical_terms_count": len(
                    content_analysis.get("technical_terms", [])
                ),
            },
...
def print_test_results(results: List[Dict]):
    ...
    print(
        f"⏱️  평균 처리 시간: {sum(r['processing_time'] for r in successful_tests) / len(successful_tests):.2f}초"
        if successful_tests
        else ""
    )
    ...
    if successful_tests:
        ...
        for result in successful_tests:
            ...
            print(
                f"   블록: {stats.get('total_blocks', 0)}개 (텍스트:{stats.get('text_blocks', 0)}, 표:{stats.get('table_blocks', 0)}, 이미지:{stats.get('image_blocks', 0)})"
            )
            print(
                f"   키워드: 주제 {content.get('main_topics_count', 0)}개, 개념 {content.get('key_concepts_count', 0)}개, 용어 {content.get('technical_terms_count', 0)}개"
            )
    ...
    if failed_tests:
        ...
        for result in failed_tests:
            print(
                f"   - {result['document_name']}: {result.get('error', '알 수 없는 오류')}"
            )
