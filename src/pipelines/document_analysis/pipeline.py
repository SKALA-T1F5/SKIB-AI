...
class DocumentAnalysisPipeline:
    ...
    def run(
        self, pdf_path: str, extract_keywords: bool = True, collection_name: str = None
    ) -> Dict[str, Any]:
        """
        문서 분석 파이프라인 실행
        ...
        if not final_collection_name:
            ...
            if not filename:
                raise ValueError(
                    f"📛 유효한 파일명을 {pdf_path}에서 추출할 수 없습니다."
                )
            final_collection_name = filename_to_collection(filename)
        ...
        print(
            f"💾 ChromaDB 업로드: {'활성화' if self.auto_upload_chromadb else '비활성화'}"
        )
        print("=" * 60)
        ...
        try:
            ...
            if self.auto_upload_chromadb:
                print(
                    f"💾 ChromaDB: {result.get('chromadb_upload_count', 0)}개 청크 업로드"
                )
...
if __name__ == "__main__":
    ...
    # Collection 명 입력
    default_collection = (
        selected_pdf.split("/")[-1].replace(".pdf", "").replace(" ", "_").lower()
    )
    collection_name = input(
        f"\nCollection 명을 입력하세요 (기본값: {default_collection}): "
    ).strip()
    if not collection_name:
        collection_name = default_collection

    # 옵션 설정
    extract_keywords = input("키워드 추출을 하시겠습니까? (y/N): ").strip().lower() in [
        "y",
        "yes",
    ]
    auto_upload = input(
        "ChromaDB 자동 업로드를 하시겠습니까? (y/N): "
