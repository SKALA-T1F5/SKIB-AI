...
from src.pipelines.base.exceptions import PipelineException, PipelineTimeoutException
...
class DocumentProcessingPipeline(BasePipeline):
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_node("error_handler", self._create_node_wrapper(self._error_handler_node))
    ...
    async def run(
        ...
        try:
            ...
            self.logger.info(f"Starting document processing for document_id: {input_data.get('document_id')}")
            self.logger.info(f"Starting document processing for document_id: {initial_state.get('filename')}")
            
            # LangGraph 실행
            config = {"configurable": {"thread_id": session_id or initial_state["pipeline_id"]}}
            result = await self.compiled_graph.ainvoke(initial_state, config)
            
            result["completed_at"] = datetime.datetime.now().isoformat()
            self.logger.info(f"Document processing completed: {result.get('processing_status')}")
            return result
    ...
    async def _parse_document_node(self, state: DocumentProcessingState) -> Dict[str, Any]:
        """문서 파싱 노드"""
        ...
        try:
            ...
            self.logger.info(f"Parsed {len(blocks)} blocks: {len(text_blocks)} text, {len(table_blocks)} tables, {len(image_blocks)} images")
    ...
    async def _analyze_content_node(self, state: DocumentProcessingState) -> Dict[str, Any]:
        """내용 분석 노드"""
        ...
        try:
            ...
            self.logger.info(f"Content analysis: {analysis_result['total_words']} words, {analysis_result['sections_count']} sections")
    ...
    async def _extract_keywords_node(self, state: DocumentProcessingState) -> Dict[str, Any]:
        """키워드 추출 노드"""
        ...
        try:
            ...
            self.logger.info(f"Keywords extracted: {len(updated_analysis.get('main_topics', []))} topics, {len(updated_analysis.get('key_concepts', []))} concepts")
    ...
    async def _store_vectors_node(self, state: DocumentProcessingState) -> Dict[str, Any]:
        """벡터 저장 노드"""
    ...
    async def _error_handler_node(self, state: DocumentProcessingState) -> Dict[str, Any]:
        """에러 처리 노드"""
        error_message = state.get("error_message", "Unknown error")
        retry_count = state.get("retry_count", 0)
        failed_step = state.get("current_step", "unknown")

        self.logger.error(f"Error handler triggered: {error_message} (retry: {retry_count}) at step: {failed_step}")
    ...
    def _create_node_wrapper(self, node_func: Callable) -> Callable:
        ...
        async def wrapper(state: DocumentProcessingState) -> Dict[str, Any]:
            ...
            try:
                self.logger.debug(f"Executing node: {node_func.__name__}", exc_info=True)
                start_time = time.time()
                
                result = await node_func(state)
                
                execution_time = time.time() - start_time
                self.logger.debug(f"Node {node_func.__name__} completed in {execution_time:.2f}s", exc_info=True)
                # self.logger.debug(f"STATE AFTER {node_func.__name__}: {result}")

                return result
                
            except Exception as e:
                self.logger.error(f"Node {node_func.__name__} failed: {str(e)}", exc_info=True)
