"""
ChromaDB 기반 테스트 생성 Pipeline
전체 워크플로우를 관리하는 ChromaDB 전용 파이프라인

워크플로우:
1. 문서 분석 Agent -> 파싱 -> ChromaDB 업로드 + 키워드/요약 생성
...
from db.vectorDB.chromadb_utils import upload_document_blocks
...
class ChromaDBTestGenerationPipeline:
    """ChromaDB 기반 테스트 생성 전체 파이프라인"""
    ...
    async def run_complete_workflow(
        ...
        Args:
            ...
            difficulty: 난이도
            upload_to_vectordb: ChromaDB 업로드 여부
        ...
        try:
            ...
            print(f"🚀 ChromaDB 테스트 생성 파이프라인 시작")
            ...
            print(
                f"📊 생성된 문제 수: {questions_result['statistics']['total_questions']}개"
            )
    ...
    async def _step1_document_analysis(
        self, pdf_path: str, collection_name: str, upload_to_vectordb: bool
    ) -> Dict[str, Any]:
        """1단계: 문서 분석 및 ChromaDB 업로드"""

        # 문서 분석 실행
        print("  📄 문서 구조 분석 중...")
        self.document_analyzer = DocumentAnalyzerAgent(collection_name)
        doc_state = self.document_analyzer.analyze_document(
            pdf_path, extract_keywords=True
        )

        if doc_state.get("processing_status") != "completed":
            raise Exception(
                f"문서 분석 실패: {doc_state.get('error_message', '알 수 없는 오류')}"
            )
        ...
        if upload_to_vectordb:
            print("  📤 ChromaDB 업로드 중...")
            uploaded_count = await self._upload_to_chromadb(
                doc_state.get("blocks", []), collection_name
            )
            ...
            "statistics": {
                ...
                "block_breakdown": {
                    "text": len(
                        [
                            b
                            for b in doc_state.get("blocks", [])
                            if b.get("type") in ["paragraph", "heading", "section"]
                        ]
                    ),
                    "table": len(
                        [
    ...
    async def _step2_test_design(
        self, doc_result: Dict[str, Any], user_prompt: str, difficulty: str
    ) -> Dict[str, Any]:
        """2단계: 테스트 설계"""
        ...
        # BaseAgent의 execute 메소드에서 결과는 output 필드에 저장됨
        result = design_state.get("output") or design_state.get(
    ...
    async def _step3_question_generation(
        self, doc_result: Dict[str, Any], design_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for i, chunk in enumerate(vision_chunks):
            if (
                generated_objective >= target_objective
                and generated_subjective >= target_subjective
            ):
                break
            ...
            try:
                ...
                from src.agents.question_generator.tools.question_generator import (
                    generate_question,
                )
                ...
                if questions:
                    ...
                    generated_objective += obj_count
                    generated_subjective += subj_count
                    print(
                        f"      ✅ {len(questions)}개 문제 생성 (객관식: {obj_count}, 주관식: {subj_count})"
                    )
    ...
    async def _upload_to_chromadb(
        self, blocks: List[Dict], collection_name: str
    ) -> int:
        """ChromaDB에 블록들 업로드"""
        try:
            # ChromaDB 업로드 (동기 함수)
            uploaded_count = upload_document_blocks(
                blocks, collection_name, f"{collection_name}.pdf"
            )
    ...
    def _extract_text_from_block(self, block: Dict) -> str:
        ...
        if block_type in ["paragraph", "heading", "section"] and content:
            ...
            # 표 데이터를 텍스트로 변환
            return self._format_table_as_text(content)
    ...
    async def _save_results(self, result: Dict[str, Any], collection_name: str):
        ...
        # 전체 결과 저장
        result_file = (
            f"{output_dir}/{collection_name}_chromadb_test_generation_result.json"
        )
        ...
        questions_only = {
            ...
            "statistics": result["questions"]["statistics"],
            "test_config": result["questions"]["test_config"],
        }

        questions_file = f"{output_dir}/{collection_name}_chromadb_questions.json"
        with open(questions_file, "w", encoding="utf-8") as f:
            json.dump(questions_only, f, ensure_ascii=False, indent=2)
        print(f"  💾 문제 저장: {questions_file}")


# 편의 함수
async def run_chromadb_pipeline(
    pdf_path: str,
    user_prompt: str,
    collection_name: str = None,
    difficulty: str = "medium",
    ...
    Returns:
        전체 처리 결과
    """
    pipeline = ChromaDBTestGenerationPipeline()
