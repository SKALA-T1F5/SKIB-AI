...
class ChromaDBTestGenerationPipeline:
    ...
    async def run_complete_workflow(
        ...
        try:
            ...
            print(f"📊 생성된 문제 수: {questions_result['statistics']['total_questions']}개")
    ...
    async def _step1_document_analysis(
        ...
        if upload_to_vectordb:
            ...
            uploaded_count = await self._upload_to_chromadb(doc_state.get("blocks", []), collection_name)
        ...
        return {
            ...
            "statistics": {
                ...
                "block_breakdown": {
                    "text": len([b for b in doc_state.get("blocks", []) if b.get("type") in ["paragraph", "heading", "section"]]),
                    "table": len([b for b in doc_state.get("blocks", []) if b.get("type") == "table"]),
                    "image": len([b for b in doc_state.get("blocks", []) if b.get("type") == "image"])
                }
            },
    ...
    async def _step3_question_generation(
        ...
        for i, chunk in enumerate(vision_chunks):
            ...
            try:
                ...
                from src.agents.question_generator.tools.question_generator import generate_question
                ...
                if questions:
                    ...
                    obj_count = len([q for q in questions if q.get("type") == "OBJECTIVE"])
                    subj_count = len([q for q in questions if q.get("type") == "SUBJECTIVE"])
                    generated_objective += obj_count
                    generated_subjective += subj_count
                    print(f"      ✅ {len(questions)}개 문제 생성 (객관식: {obj_count}, 주관식: {subj_count})")
    ...
    async def _upload_to_chromadb(self, blocks: List[Dict], collection_name: str) -> int:
        """ChromaDB에 블록들 업로드"""
        try:
            # ChromaDB 업로드 (동기 함수)
            uploaded_count = upload_document_blocks(blocks, collection_name, f"{collection_name}.pdf")
    ...
    async def _save_results(self, result: Dict[str, Any], collection_name: str):
        ...
        # 전체 결과 저장
        result_file = f"{output_dir}/{collection_name}_chromadb_test_generation_result.json"
