...
def parse_pdf_unified(pdf_path: str, collection_name: str = None, generate_questions: bool = False, num_objective: int = 3, num_subjective: int = 3) -> List[Dict]:
    """
    ...
    if generate_questions:
        ...
        all_blocks = _generate_questions_for_blocks(all_blocks, IMAGE_SAVE_DIR, num_objective, num_subjective)
...
def _generate_questions_for_blocks(blocks: List[Dict], image_save_dir: str, num_objective: int, num_subjective: int) -> List[Dict]:
    """블록들을 GPT-4 Vision 메시지로 변환하여 질문 생성"""
    try:
        # QuestionGenerator 사용
        question_generator = QuestionGenerator(image_save_dir)
        return question_generator.generate_questions_for_blocks(blocks, num_objective, num_subjective)
    except Exception as e:
        print(f"❌ 질문 생성 중 오류: {e}")
        return blocks


def _blocks_to_vision_chunks(blocks: List[Dict], image_save_dir: str, max_chunk_size: int = 15000) -> List[Dict]:
    ...
    def save_current_chunk():
        if current_chunk['messages']:
            ...
            final_metadata['page'] = final_metadata['pages'][0] if final_metadata['pages'] else 1
    ...
    for block_idx, block in enumerate(blocks):
        ...
        # 청크 크기 확인 및 저장
        if message_content:
            if current_chunk['current_length'] + text_length > max_chunk_size and current_chunk['messages']:
                save_current_chunk()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(pdf_path):
            ...
            blocks = parse_pdf_unified(pdf_path, collection_name, generate_questions=generate_questions)
        ...
        print("사용법: python unified_parser.py <pdf_path> [collection_name] [generate_questions:true/false]")
