...
class TestDesignerAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        설정된 도구들:
        - requirement_analyzer: 요구사항 분석 도구
        - config_generator: 테스트 설정 생성 도구
        """
        super().__init__(
            name="test_designer",
    ...
    async def plan(self, input_data: Dict[str, Any], state: BaseState) -> Dict[str, Any]:
        ...
        Args:
            input_data: 입력 데이터 (사용자 요청, 키워드, 문서 요약 등)
            state: 현재 상태
...
"""
        ...
        try:
            ...
            full_prompt = f\"\"\"당신은 교육 평가 전문가입니다. 주어진 정보를 바탕으로 명확하고 구체적인 테스트 요약을 작성합니다.
