...
def _save_test_plans(result: Dict[str, Any], documents: List[Dict[str, Any]]):
    ...
    if "test_summary" in result:
        ...
        total_test_plan = {
            "metadata": {
                ...
                "document_names": [doc.get("document_name", f"문서_{doc.get('document_id', i+1)}") 
            ...
            "aggregated_info": {
                "all_keywords": list(set([kw for doc in documents for kw in doc.get("keywords", [])])),
                "all_summaries": [doc.get("summary", "") for doc in documents],
                "all_topics": list(set([topic for doc in documents for topic in doc.get("main_topics", [])]))
            }
        }
    ...
    if "test_summary" in result and "document_configs" in result["test_summary"]:
        ...
        for config in document_configs:
            # 원본 문서 정보 찾기
            original_doc = next((doc for doc in documents if doc.get("document_id") == config.get("document_id")), {})
            
            doc_plan = {
                "document_id": config.get("document_id"),
                "document_name": original_doc.get("document_name", f"문서_{config.get('document_id')}"),
                ...
                "recommended_questions": {
                    ...
                    "total": config.get("recommended_objective", 5) + config.get("recommended_subjective", 3)
                }
            }
