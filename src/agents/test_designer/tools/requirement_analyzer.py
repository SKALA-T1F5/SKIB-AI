...
class RequirementAnalyzer:
    ...
    def analyze(
        self, user_prompt: str, keywords: List[str], document_summary: str
    ) -> Dict[str, Any]:
        """요구사항 종합 분석"""
    ...
    def _extract_test_type(self, prompt: str) -> str:
        ...
        has_objective = any(
            keyword in prompt_lower for keyword in self.test_type_keywords["objective"]
        )
        has_subjective = any(
            keyword in prompt_lower for keyword in self.test_type_keywords["subjective"]
        )
    ...
    def _extract_question_count(self, prompt: str) -> Dict[str, int]:
        ...
        # 전체 문제 수만 언급된 경우
        total_patterns = [r"총.*?(\d+).*?문", r"(\d+).*?문제", r"문제.*?(\d+).*?개"]
    ...
    def _extract_time_limit(self, prompt: str) -> int:
        """제한시간 추출 (분 단위)"""
        time_patterns = [r"(\d+)분", r"(\d+)시간", r"제한시간.*?(\d+)", r"시간.*?(\d+)"]
