...
def generate_question(
    ...
    try:
        ...
        print(f"  🤖 GPT-4 Vision 호출 중... (객관식: {num_objective}, 주관식: {num_subjective})")
    ...
    def generate_questions_for_blocks(
        ...
        try:
            ...
            for i, chunk in enumerate(vision_chunks):
                ...
                # 남은 질문 수 계산
                remaining_obj = max(0, num_objective - len([
                    q for b in blocks for q in b.get('questions', []) 
                    if q.get('type') == 'OBJECTIVE'
                ]))
                remaining_subj = max(0, num_subjective - len([
                    q for b in blocks for q in b.get('questions', []) 
...
def generate_questions_for_document(
    ...
    return generator.generate_questions_for_blocks(blocks, num_objective, num_subjective)