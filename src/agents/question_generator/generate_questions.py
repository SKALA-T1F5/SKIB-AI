...
def localize_enum_fields(question: Dict) -> Dict:
    ...
    if difficulty_key_original:
        ...
        question["difficulty_level"] = difficulty_map.get(
            question[difficulty_key_original], question[difficulty_key_original]
        )
        if (
            difficulty_key_original != "difficulty_level"
            and difficulty_key_original in question
        ):  # 이미 difficulty_level로 바뀌었으면 삭제 불필요
...
def generate_question(
    ...
    for q_idx, q_item in enumerate(parsed_questions):
        if not isinstance(q_item, dict):
            print(
                f"Warning: Skipping non-dict item in parsed list at index {q_idx}: {str(q_item)[:100]}..."
            )
            continue
        ...
        if missing_keys:
            print(
                f"Warning: Skipping question due to missing keys {missing_keys} in item: {str(q_item)[:100]}..."
            )
            continue

        if q_item.get("type") == "OBJECTIVE" and "options" not in q_item:
            print(
                f"Warning: Skipping OBJECTIVE question due to missing 'options': {str(q_item)[:100]}..."
            )
            continue
    ...
    if not processed_questions and parsed_questions:
        ...
        return [
            {
                "error": "All parsed questions were filtered out during post-processing.",
