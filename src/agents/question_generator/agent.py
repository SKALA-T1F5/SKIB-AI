...
class QuestionGeneratorAgent:
    ...
    def _extract_used_keywords(self, question: Dict, available_keywords: List[str]) -> List[str]:
        """
        문제에서 실제 사용된 키워드만 추출
        ...
        for keyword in available_keywords:
            ...
            elif keyword.lower().replace(' ', '').replace('_', '').replace('-', '') in combined_text.replace(' ', '').replace('_', '').replace('-', ''):
                used_keywords.append(keyword)
