...
from .tools.save_results import save_question_result
import os
import json
    ...
    def _save_question_results(
        ...
        questions_data = {
            ...
            "question_summary": {
                ...
                "objective_questions": len([q for q in questions if q.get('type') == 'OBJECTIVE']),
                "subjective_questions": len([q for q in questions if q.get('type') == 'SUBJECTIVE']),
        ...
        # 3. 테스트 config 파일 생성
        config_file = self._save_test_config(
            questions, source_file, timestamp
        )
        if config_file:
        ...
        return {
            ...
            "questions": questions,
            "total_questions": len(questions),
            "objective_count": len([q for q in questions if q.get('type') == 'OBJECTIVE']),
            "subjective_count": len([q for q in questions if q.get('type') == 'SUBJECTIVE']),
        ...
        try:
            ...
            test_summary_data = {
                ...
                "content_analysis": {
                    ...
                    "content_complexity": self._analyze_content_complexity(keywords, main_topics, questions)
                },
                "test_structure": {
                    "total_questions": len(questions),
                    "question_breakdown": {
...
def generate_questions_from_document(
    ...
    return agent.generate_questions_from_blocks(
        blocks, num_objective, num_subjective, source_file, keywords, main_topics, summary
    )