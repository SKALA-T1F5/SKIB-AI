...
class BaseAgent(ABC):
    ...
    async def execute(
        self, input_data: Dict[str, Any], shared_state: Optional[BaseState] = None
    ) -> BaseState:
        """
        Agent의 메인 실행 메서드
        ...
        try:
            ...
            # Agent 실행 플로우
            await self._execute_workflow(input_data)

            # 성공 시 상태 업데이트
            update_state_progress(
                self._current_state, progress=1.0, status=StateStatus.COMPLETED
            )
    ...
    async def _execute_workflow(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if is_valid:
                    self.logger.info(
                        f"Agent {self.name} completed successfully on attempt {attempt + 1}"
                    )
                    ...
                    self.logger.warning(
                        f"Agent {self.name} validation failed on attempt {attempt + 1}: {feedback}"
                    )
                    if attempt < max_retries:
                        ...
                        raise ValueError(
                            f"Agent failed validation after {max_retries} attempts: {feedback}"
                        )

            except Exception as e:
                if attempt < max_retries:
                    self.logger.warning(
                        f"Agent {self.name} attempt {attempt + 1} failed: {e}. Retrying..."
                    )
    ...
    def update_progress(self, progress: float, message: Optional[str] = None) -> None:
        ...
        if self._current_state:
            update_state_progress(
                self._current_state, progress, current_agent=self.name
            )
            if message:
                add_state_log(
                    self._current_state, level="INFO", message=message, agent=self.name
                )
    ...
    def add_warning(self, message: str) -> None:
        ...
        if self._current_state:
            ...
            add_state_log(
                self._current_state, level="WARNING", message=message, agent=self.name
            )
...
class MockAgent(BaseAgent):
    ...
    async def reflect(
        ...
        feedback = (
            "Mock validation successful" if is_valid else "Mock validation failed"
        )
