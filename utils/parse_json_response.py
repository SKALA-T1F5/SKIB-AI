...
def parse_json_response(raw_content: str) -> Union[Dict, List[Dict]]:
    ...
    code_block_match = re.search(
        r"```(?:json)?\s*(.*?)\s*```", text_to_parse, re.DOTALL | re.IGNORECASE
    )
    if code_block_match:
        ...
        start_idx = -1
        if first_bracket_idx != -1 and (
            first_curly_idx == -1 or first_bracket_idx < first_curly_idx
        ):
    ...
    try:
        ...
        try:
            ...
            if (
                isinstance(outer_obj, dict)
                and "generated_questions" in outer_obj
                and isinstance(outer_obj["generated_questions"], list)
            ):
    ...
    if isinstance(parsed_data, list):
        ...
        if "generated_questions" in parsed_data and isinstance(
            parsed_data["generated_questions"], list
        ):
        ...
        raise ValueError(
            f"JSON 파싱은 성공했으나, 결과가 리스트 또는 딕셔너리 형식이 아닙니다. 얻은 타입: {type(parsed_data)}"
        )
